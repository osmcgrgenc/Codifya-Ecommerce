...
export default function CheckoutContent({ user }: CheckoutContentProps) {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const endpoint = paymentMethod === "iyzico" 
        ? "/api/payment/iyzico" 
      ...
      if (paymentMethod === "bank-transfer") {
        ...
        router.push(`/orders/bank-transfer?orderId=${data.orderId}&referenceCode=${data.referenceCode}`);
      }
    } catch (error) {
  ...
  const validateForm = () => {
    ...
    if (!sameAsShipping && (!billingAddress.contactName || !billingAddress.address || !billingAddress.city)) {
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          ...
          <Tabs defaultValue="iyzico" onValueChange={setPaymentMethod}>
            ...
            <TabsContent value="iyzico" className="mt-4">
              <div className="p-4 bg-gray-50 rounded-md">
                <p className="text-sm text-gray-600">
                  ...
                  Ödeme onayından sonra, kredi kartı bilgilerinizi girmeniz için İyzico ödeme sayfasına yönlendirileceksiniz.
                </p>
            ...
            <TabsContent value="bank-transfer" className="mt-4">
              <div className="p-4 bg-gray-50 rounded-md">
                <p className="text-sm text-gray-600">
                  Banka havalesi veya EFT ile ödeme yapmayı seçtiniz. Siparişiniz onaylandıktan sonra,
                  size özel bir referans kodu ve banka hesap bilgileri verilecektir. Ödemenizi yaparken
                  bu referans kodunu belirtmeyi unutmayın.
                </p>
                <div className="mt-4">
      ...
      <div className="md:col-span-1">
        <div className="bg-white p-6 rounded-lg shadow-sm border sticky top-4">
          ...
          <p className="text-xs text-gray-500 mt-4 text-center">
            Ödemeyi tamamlayarak, Satış Sözleşmesi ve Gizlilik Politikası&apos;nı kabul etmiş olursunuz.
          </p>
        </div>
      </div>
    </div>
