...
export async function POST(req: NextRequest) {
  try {
    ...
    if (paymentResult.status !== "success" || paymentResult.paymentStatus !== "SUCCESS") {
      ...
      return NextResponse.redirect(
        new URL(`/payment/error?message=${encodeURIComponent(paymentResult.errorMessage || "Ödeme işlemi başarısız oldu")}`, req.url)
      );
    }
    ...
    return NextResponse.redirect(
      new URL(`/orders/${orderId}?success=true`, req.url)
    );
  } catch (error) {
